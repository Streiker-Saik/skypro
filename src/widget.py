...
def mask_account_card(number_card_account: str) -> str:
    ...
    if number_card_account.count(" ") == 1:
        index_spice = number_card_account.index(" ")
    else:
        index_spice = number_card_account.find(" ", number_card_account.find(" ") + 1)
    return (
        f"{number_card_account[:index_spice]} {get_mask_account(number_card_account)}"
        if number_card_account[:index_spice] == "Счет"
        else f"{number_card_account[:index_spice]} {get_mask_card_number(number_card_account)}"
    )
